...
totalBiomassPerSpecies <- function(dataPath,
  ...
  biomassBySpecies <- rbindlist(lapply(X = names(cohortDataList), FUN = function(yr) {
    ...
    thisPeriod <- pixelCohortData[, list(
      year = as.numeric(substrBoth(
        strng = yr,
        howManyCharacters = 4,
        fromEnd = TRUE
      )),
  ...
  png(
    filename = file.path(dataPath, paste0("biomassMapStack_", typeSim, prop, overS, ".png")),
    height = 600, width = 900
  )

  if (columnsType) {
    plot2 <- ggplot(
      data = biomassBySpecies,
      aes(x = year, y = y, fill = speciesCode), position = "fill"
    ) +
      geom_col(aes(y = y)) +
      scale_fill_viridis_d() +
      labs(x = "Year", y = "Total Biomass", title = paste0(
    ...
    plot2 <- ggplot(data = biomassBySpecies, aes(
      x = year, y = y,
