...
#' @rdname createModels
createModels <- function(caribouCoefTable = NULL, modelsToUse = "M3") {
  message("Using model ", crayon::magenta(modelsToUse), " for caribou...")
  if (is.null(caribouCoefTable)) {
    message(yellow("caribouCoefTable was not supplied. Using defauilt from ECCC2011 for demographics"))
    caribouCoefTable <- structure(list(
      ResponseVar = c(
        "Rec", "Rec", "Rec", "Rec", "Rec",
        "Rec", "Rec", "Rec", "Rec", "Rec", "Rec", "Rec", "Rec", "Rec"
      ),
      ModelNum = c(
        "M3", "M3", "M7", "M7", "M7", "M8", "M8", "M8",
    ...
    row.names = c(NA, -14L), spec = structure(list(
      cols = list(
        ResponseVar = structure(list(), class = c(
          "collector_character",
          "collector"
        )),
        ModelNum = structure(list(), class = c(
          "collector_character",
          "collector"
        )),
        ...
        Coefficient = structure(list(), class = c(
          "collector_character",
          "collector"
        )),
