...
reviseSpeciesTraits <- function(speciesTable = NULL, updatedTable = NULL,
                                ...
                                returnTable = FALSE,
                                species = c(
                                  "BETU.PAP", "LARI.LAR", "PICE.GLA", "PICE.MAR",
                                  "PINU.BAN", "POPU.TRE", "PINU.CON"
                                )) {
  if (returnTable) {
    message(crayon::blue(paste0(
      "This is the ORIGINAL species table traits (from LANDIS-II).",
    ...
    invisible(lapply(X = speciesToChange, function(sp) {
      lapply(X = paramsToChange, FUN = function(param) {
        speciesTable[species == sp, (param) := updatedTable[
          species == sp,
