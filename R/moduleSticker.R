utils::globalVariables(c("module", "parameter", "value"))
...
moduleSticker <- function(moduleTable = NULL, moduleName, directory = getwd(),
  ...
  if (is.null(moduleTable)) {
    moduleTable <- prepInputs(
      targetFile = "moduleTable.csv",
  ...
  imageURL <- paste0(
    "https://github.com/tati-micheletti/host/raw/master/images/",
    moduleTable[parameter == "imageName", value], ".png"
  )
  if (is.null(savedSticker)) {
    savedSticker <- file.path(directory, paste0(
      "stickers/",
