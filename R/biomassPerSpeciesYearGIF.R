...
biomassPerSpeciesYearGIF <- function(dataPath, years = NULL, uploadTo) {
  ...
  cohortDataComplete[, c("medianBiomass", "totalBiomass", "meanBiomass", "seBiomass", "sdBiomass") :=
    list(
      median(as.numeric(BiomassBySpecies), na.rm = TRUE),
  ...
  cols <- c(
    "speciesCode", "year", "totalBiomass", "medianBiomass",
  ...
  p <- lapply(
    X = c("totalBiomassPerSpecies", "medianBiomassPerSpecies", "meanBiomassPerSpecies"),
    function(plotting) {
      gifName <- file.path(dataPath, paste0(
        plotting,
