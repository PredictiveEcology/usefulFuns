utils::globalVariables(c(
  "..column", "direction", "ID", "id", "location",
...
.calculateSignificantChangesInBirds <- function(dataPath, years, species = NULL, pixelBased = TRUE,
  ...
  if (is.null(species)) {
    ...
    splittedAllNames2 <- splittedAllNames[!splittedAllNames %in% grepMulti(
      x = splittedAllNames,
      patterns = paste0("_", years[1])
    )]
    splitted <- unlist(lapply(splittedAllNames, function(birdFilename) {
      if (length(birdFilename) == 1) {
        return(NULL)
      }
  ...
  tableOfChanges <- data.table::rbindlist(lapply(species, function(bird) {
    birdInyears <- lapply(years, function(y) {
      ...
      if (length(rasPath) == 0) {
        stop(
          "The raster file for ", bird, " for year ", y, " wasn't found. Maybe not all species were run for all years? ",
    ...
    if (!is.null(studyArea)) {
      ...
      dt <- data.table::rbindlist(lapply(uniqueLocations, function(loc) {
        ...
        dt <- data.table::data.table(
          species = bird, tTest = byLocationModelList$p.value,
          result = direction, location = loc
        )
      }))
    } else {
      r <- .calculatePvalueOfRasters(
        dtForTest = dtForTest,
        pixelBased = pixelBased, sampleSize = sampleSize,
        species = species,
        redFactorTimes = redFactorTimes, bird = bird
  ...
  if (pixelBased) {
    if (!is.null(sampleSize)) {
      if (all(sampleSize == "auto", bird == species[[1]])) {
        ...
        ## data and factor
        cohen <- cohen.d(treatment, control,
          paired = TRUE,
        ...
        while (cohen$estimate < limit) {
          ...
          cohen <- cohen.d(treatment[newSample], control[newSample],
            paired = TRUE,
            conf.level = 0.01, hedges.correction = TRUE
          )
        ...
        message(crayon::green(paste0(
          "Ideal sample size calculated as ",
          length(sampleSize), "\n with sample size effect calcutated as"
        )))
        print(cohen)
        stop(paste0(
          "Please pass the ideal size calculated", length(sampleSize),
      ...
      if (!exists("sbsetID")) {
        if (tryCatch(
          ...
          assign(
            x = "sbsetID", value = sample(
              x = dtForTest[!is.na(dtForTest)[ID], ID],
...
.prepStudyAreaForBirds <- function(studyArea, dataPath) {
  if (is(studyArea, "character")) {
    studyArea <- prepInputs(
      url = studyArea, targetFile = "birdRTMEdehzhie.tif",
...
.calcICPercentChange <- function(percChange) {
  ...
  dt <- data.table::data.table(
    direction = c("increased", "decreased", "noChange"),
